...
若要了解当前界面、原布局区别与开启方式，请先阅读 [Essay 随笔界面与使用指南](./essay-layout.md)。本文侧重变更历史与检查记录。

本文记录随笔系统的实现与后续调整，供人工检查。第 1–7 节为首次实现的历史记录，第 8 节记录后续组件字体调整。
...
根据“正文中英文字体舒服，但组件字体不协调”的反馈，确认后进行了以下修改。

### 修改文件与作用

- `src/assets/styles/essay.css`
  - 保留正文的字体栈、字号、行高及原有标题样式，不改文章内容。
  - 组件标签、章节编号、公式术语、图表说明与英文折叠标题改用正文的衬线字体；能通过继承实现的地方直接移除字体覆盖。
  - 移除等宽字体变量；章节和目录编号使用衬线字体，并请求齐线、等宽数字特性，而不是将整个字体切换为代码字体。
  - 标签从约 10.9px 提高到 14px，移除强制全大写和加宽字距。原文自己写成大写的开篇眉题仍保持原样。
  - 图表注释和折叠提示改为 13px，其他主要辅助文字改为 14px。
  - 折叠标题中英文之间的间距略微缩小，为较大的字体留出空间。
  - 数学公式仍使用 KaTeX；页面导航、日期、页尾等功能区保留无衬线字体。
- `src/components/essay/README.md`：补充当前字体规则，便于后续复用保持一致。
- `docs/future-ai-changes.md`：增加本节变更记录。前文的“无衬线组件标签／等宽术语”描述属于首次实现，不再代表当前组件样式。

### 验证

- 生产构建完成；Pagefind 平台包安装警告仍存在，未在此次处理。
- 重跑 320–1440px 页面布局、英文折叠、目录跳转、深色模式及公式滚动检查，通过。
- 浏览器计算样式检查确认：上述组件的字体族与正文一致，辅助文字至少 13px，标签不再强制大写或加宽字距。
- 查看桌面对比图与手机公式截图，确认字号调整后的布局。
- 外部网络集成仍按第 5 节所述在测试中被阻断，不属于此次验证范围。

未新增项目依赖，未加载新的字体文件，未修改组件结构或文章正文。
